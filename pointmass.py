...
def get_model_and_assets():
  ...
  return common.read_model('pointmass.xml'), common.ASSETS
...
class PointMass(base.Task):
  ...
  def get_reward(self,physics):
      a=0
      b=1
      cyl_obs_dist = physics.named.data.geom_xpos['cylinder']-physics.named.data.geom_xpos['target']
      dist = np.linalg.norm(cyl_obs_dist)
      cyl_ball_dist = physics.named.data.geom_xpos['cylinder']-physics.named.data.geom_xpos['pointmass']
      dist2 = np.linalg.norm(cyl_ball_dist)
      rew=a*dist+b*dist2
      rew = -1.0 * rew  # negative cost is reward
      #return rew
