...
class DMCWrapper(core.Env):
    ...
    def render(self, mode='rgb_array', height=None, width=None, camera_id=0):
        ...
        #print(camera_id)
        # self.pixels = self._env.physics.render(height=self._height, width=self._width, camera_id=self._camera_id)
        #
        # self._get_viewer(mode).update(self.pixels)
        return self._env.physics.render(
            height=height*3, width=width*3, camera_id=camera_id
        )
    ...
    def _get_viewer(self, mode):
        h= DmControlViewer(self.pixels.shape[1], self.pixels.shape[0])
        return h